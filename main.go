...
import (
	...
	"github.com/zongrade/rasp/internal/test"
)
...
// short нужно, для телефонов, иначе не влезает в экран
func ArrayWeekDString(lang string, short bool) (slice []string) {
	switch lang {
	case "ru":
		if short {
			//TODO: переписать через итерацию по массиву
			slice = []string{
				Monday.RuShortString(),
				Tuesday.RuShortString(),
				Wednesday.RuShortString(),
				Thursday.RuShortString(),
				Friday.RuShortString(),
				Saturday.RuShortString(),
				Sunday.RuShortString(),
			}
		} else {
			slice = []string{
				Monday.RuString(),
...
func (day WeekD) RuShortString() string {
	switch day {
	case Monday:
		return "Пн"
	case Tuesday:
...
func main() {
	//TODO: раскоментировать, чтобы заработало
	//try()
	appT.TestFindPrepod()
	//test.Test()
	//another.TestAnotherFyne()
}
...
func initializeData(VT *VirtualTree, data *DaAr, textState *widget.Label) error {
	...
	if ext, jsonPath, globalError := isDataJsonExist2(VT.App); globalError != nil {
		...
		bArr, globalError := net.GetRaspisanie(ch, "П-21М")
		if globalError != nil {
			return globalError
		} else {
...
func try() {
	...
	w := a.NewWindow("Расписание")
	//TODO: сделать стартовое окно с выбором группы
...
func createArrButtonRaspByDay(data *DaAr, content *fyne.Container, VT *VirtualTree, w *fyne.Window) (slice []fyne.CanvasObject) {
	weekDs := ArrayWeekD()
	for i, dayName := range ArrayWeekDString("ru", true) {
		buffI := i
		buffDay := dayName
...
//TODO: сделать горутину наблюда.щую за скроллом и доводящую вниз
...
func getJson(ch chan<- struct{}, group string) ([]byte, error) {
	...
	url := "https://miet.ru/schedule/data?group=" + group
